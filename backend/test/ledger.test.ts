...
// the rules that make the ledger trustworthy: one active pact per
// user, idempotent event ingestion, and no writes to a closed pact.

const DATABASE_URL = process.env.DATABASE_URL;

describe.skipIf(!DATABASE_URL)("pact ledger", async () => {
  const { db } = await import("@/server/db/client");
  const { registerDevice } = await import("@/server/devices");
  const { createPact, getCurrentPact, listPacts } = await import("@/server/pacts");
  ...
  it("locks one pact and refuses a second while it is active", async () => {
    ...
    await expect(
      createPact(userId, { device_id: deviceId, duration_days: 1, timezone: "UTC", snapshot }),
    ).rejects.toMatchObject({ status: 409, code: "pact_active" });

    expect((await getCurrentPact(userId))?.id).toBe(c.id);
  });

  it("ingests an event once, returns the same row on retry, closes the pact, queues witness rows", async () => {
    const current = (await getCurrentPact(userId))!;
    const eventId = newId();
    ...
    const closed = await db.selectFrom("pact").select(["status", "ended_at"]).where("id", "=", current.id).executeTakeFirstOrThrow();
    expect(closed.status).toBe("broken");
    expect(closed.ended_at).not.toBeNull();

    const events = await db.selectFrom("pact_event").select("type").where("pact_id", "=", current.id).execute();
    ...
    expect(queued).toEqual([{ recipient_id: witnessId, kind: "pact_broken", status: "queued" }]);
    expect(await getCurrentPact(userId)).toBeUndefined();
  });

  it("refuses new events on a closed pact but still answers retries", async () => {
    const closed = (await listPacts(userId, undefined, 10)).items[0]!;
    await expect(
  ...
  it("refuses an event id that belongs to another pact", async () => {
    const closed = (await listPacts(userId, undefined, 10)).items[0]!;
    const usedId = (await db.selectFrom("pact_event").select("id").where("pact_id", "=", closed.id).where("type", "=", "broken").executeTakeFirstOrThrow()).id;
    const next = await createPact(userId, { device_id: deviceId, duration_days: 1, timezone: "UTC", snapshot });
    await expect(
  ...
  it("does not let another user read or write the pact", async () => {
    const other = newId();
    const current = (await getCurrentPact(userId))!;
    await expect(
  ...
  it("pages history newest first with a cursor", async () => {
    const page1 = await listPacts(userId, undefined, 1);
    expect(page1.items).toHaveLength(1);
    expect(page1.next_cursor).toBe(page1.items[0]!.id);
    const page2 = await listPacts(userId, page1.next_cursor!, 1);
