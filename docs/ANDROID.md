...
| `cycling` | GPS through `LocationManager`, read by `RideService` (a location-typed foreground service) only while a ride runs; needs `ACCESS_FINE_LOCATION` | Each fix against the last for speed and distance; metres at 8 to 45 km/h count, with the step cadence under 100/min so a run is not a ride. 3 km. No route kept. `android/CYCLING.md` |
