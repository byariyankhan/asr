...
create index commitment_user_status_idx on commitment (user_id, status);
create index commitment_active_ends_idx on commitment (ends_at) where status = 'active';
-- at most one active commitment per user
create unique index commitment_one_active_idx on commitment (user_id) where status = 'active';
```
...
create table commitment_event (
  ...
  minutes         integer,                       -- reward minutes for challenge_completed
  occurred_at     timestamptz not null,          -- device clock, untrusted
  received_at     timestamptz not null default now(),
  source          text not null check (source in ('device', 'server')),
  created_at      timestamptz not null default now()
);
create index commitment_event_commitment_idx on commitment_event (commitment_id, received_at desc);
```
...
create index challenge_pending_deadline_idx on challenge (deadline_at) where status = 'pending';
create index challenge_commitment_idx on challenge (commitment_id);
```
...
create table notification (
  ...
  about_user_id  text references "user"(id) on delete set null,   -- whose commitment this is about
  event_id       uuid references commitment_event(id) on delete set null,
  channel        text not null check (channel in ('push', 'email')),
  kind           text not null,                -- 'commitment_started', 'commitment_broken', ...
  title          text not null,
  body           text not null,
  deep_link      text,
...
Sent once per day by the phone, only while a commitment is active. This is
...
create table daily_summary (
  commitment_id    uuid not null references commitment(id) on delete cascade,
  day              date not null,               -- in the commitment's timezone
...
- No `streak` table. Streaks are derived from `commitment` and
  `commitment_event` (a query, cached in Redis for the witness view).

## Retention

| Data | Kept |
|---|---|
| `commitment`, `commitment_event`, `challenge` | For the life of the account (it is the user's history) |
